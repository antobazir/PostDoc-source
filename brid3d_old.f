...
!- start picking cells according to Bridson algorithm
      do k=2,numc ! loop sweeping from k to number of cells
      	reject = 0
      	call cpu_time(start_outer)
      	m0 = 0
  99  	continue ! goes to 99 only if told to
      	rr=rad*(1.+rand()) !rr defines radial coordinates between 1Rc and 2Rc
      	fi=duepi*rand() ! defines a random angle between 0 and 2pi
      	xi=pigr*(-1.d0+2.d0*rand()) ! defines random value between -pi and pi
      	if (m0.eq.30) then
      		ii=rand()*k 
      		m0=0
      	end if
      !ii=(0.4 + 0.6*rand())*k ! defines a random number between  0 and k and rounds it as it is an integer -> random selection of point in space 
