...
function [G,O,Gt,Ot, kGt, kOct, kG, kO, Grid, sycle] = cfsys2DGOr_cc_sym(conf)
...
for i=1:ntime
    ...
    if(mod(i,1000)==0)
		...
		for n=1:length(sycle)
			sycle(m,2) = interp1([0.150 0.0375 0.015],[2.8000-0.6 6.6000-0.6 400.0000-0.6]*24*60,O(find(Grid==m)))+864;
		endfor

		for l=1:length(sycle)
			%l

			if(sycle(l,1)==sycle(l,2))
				[r,c] = ind2sub([round(sz/2)+1,round(sz/2)+1],find(Grid==l))
				pos(1) = r; pos(2) = c;
				[Grid,sycle] = divide2D_cc(Grid,pos,m,round(sz/2)+1,G,O,sycle);
			endif
		endfor
	endif
