...
\vspace{0.3cm} 
\[ TFD\{ x[n] \} =  \sum_{k = 0}^{N-1} x[k] \; e^{-j 2 \pi k \frac{m}{N}}  \] \\
\vspace{0.3cm}
\only<2->{
\begin{itemize}
...
\begin{frame}
\frametitle{Signal porte} 
On échantillonne le signal porte sur 9 valeurs dont 5 non-nulle
\begin{columns}
\column{60mm}
\begin{center}
\begin{tikzpicture}
...
\only<2->{
\vspace{0.5cm}
\[ TFD\{ x[n] \} =  \frac{1}{N} \sum_{k = 0}^{N-1} x[n] \; e^{-j 2 \pi n \frac{m}{N}}  \]\\
\vspace{0.5cm}
}
\only<3->{
\[ TFD\{ \Pi_5[n] \} = \tilde{\Pi_5}[m]  = \sum_{k = 0}^{8} \Pi_5[k] \; e^{-j 2 \pi k \frac{m}{9}}  \]\\
\vspace{0.5cm}
}
\only<4->{
...
\draw[blue,thick] (0,0.55*4)node[left] {0.55}--(0.5,0.32*4)--(1,0.06*4)--(1.5,0.11*4)--(2,0.07*4)--(2.5,0.07*4)--(3,0.11*4)--(3.5,0.06*4)--(4,0.32*4);
...
Comme auparavant, on prend une porte non-nulle sur 5 échantillons,\\
...
\vspace{0.3cm}
\only<4->{
Cette fois-ci le spectre est symétrique ET périodique de période $f_e$
}
\end{frame}
...
\only<2->{
\draw[very thick,domain=-3/7:(3/7),color=blue,samples=7] plot[ycomb] (\x,{0.2*abs(1 + 2*cos(2*3.14*\x r) + 2*cos(4*3.14*\x r)) });
