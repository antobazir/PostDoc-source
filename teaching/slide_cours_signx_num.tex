...
\[TZ\{ x \}(z) = \sum_{n = -\infty}^{\infty} s_{T_e}[n] z^{-n} \]
}
 \vspace{0.3 cm}
...
\[TZ\{ x \}(z) = \sum_{n = 0}^{\infty} s_{T_e}[n] z^{-n} \]
}
...
\vspace{0.3 cm} 
\only<3->{
La transformée en $z$ est une fonction complexe
}
...
\item<3-> Utilisation possible du zéro padding pour de meilleures courbes.

\end{itemize}
\vspace{1cm}
Dans tous ces développements on part du principe que l'amplitude est continue mais ce n'est pas forcément vrai (codage)
\end{frame}
