...
\only<2->{
\small{Que faut-il avoir à l'esprit lors du choix de $f_e$ (fréquence d'échantillonnage)} ?
}
...
\begin{scope}[scale=0.4]
	\draw[->] (-5,0)-- (5,0);
%\draw (-0,-0.8) node {\scriptsize 0};
\draw[->] (0,0)-- (0,3);
\draw (5.2,0.25) node {\scriptsize $\nu$};
...
\begin{scope}[scale=0.4]
	\draw[->] (-7,0)-- (7,0);
%\draw (-0,-0.4) node {\scriptsize 0};
\draw[->] (0,0)-- (0,3);
\draw (7.2,0.25) node {\scriptsize $\nu$};
\draw (0,3.5) node {\scriptsize $|G(\nu)|$};
%\draw (4.5,-0.3) node {1};
...
\begin{frame}
\frametitle{Illustration du repliement de spectre}
\small{Echantillonnons le même signal à une fréquence de $\frac{3}{\pi}$}
\begin{columns}
\column{60mm}
\begin{center}
\begin{tikzpicture}
...
\draw[domain=-4.9:4.9,color=orange,samples=12] plot (\x,{2*(0.55*cos(2*\x r)+ 0.45*cos(2*2*(\x+3.14/12) r))});

\draw[dotted,domain=-4.9:4.9,color=blue,samples=160] plot (\x,{2*(0.55*cos(2*\x r)+ 0.45*cos(2*2*(\x+3.14/12) r))});

\draw[dashed,domain=-4.9:4.9,color=blue,samples=12] plot[ycomb] (\x,{2*(0.55*cos(2*\x r)+ 0.45*cos(2*2*(\x+3.14/12) r))});
...
%replica 1
\draw[thick,orange] (5.05-0.5,0)-- (5.05-0.5,2.5*0.55);
\draw[thick,orange] (6.05-0.5,0)-- (6.05-0.5,2.5*0.45);
%\draw (5.1,-0.4) node {$\frac{5}{\pi}$};
%\draw (6.1,-0.4) node {$\frac{6}{\pi}$};

\draw[thick,orange] (3.05-0.5,0)-- (3.05-0.5,2.5*0.55);
\draw[thick,orange] (2.08-0.5,0)-- (2.08-0.5,2.5*0.45);
%\draw (3.1,-0.4) node {$\frac{3}{\pi}$};
%\draw (2.1,-0.4) node {$\frac{1}{\pi}$};
\draw (4.1-0.5,-0.8) node { \scriptsize $\frac{3}{\pi}$};
...
\only<3->{
\begin{block}{}
Les composantes des répliques et du spectre "original" se mélangent...
\end{block}
}
...
\begin{frame}
\frametitle{Illustration du repliement de spectre}
\small{Comment comprendre le repliement ?}
\begin{columns}
\column{60mm}
\begin{center}
\begin{tikzpicture}
...
\small{
\begin{itemize}
\item Le signal échantillonné "manque" des alternances
\vspace{0.3cm}
\item Les composantes des répliques "ajoutent" des basses fréquences
\vspace{0.3cm} 
\item Le décalage se verra aussi en déphasage
\end{itemize}
}
...
\begin{frame}
\frametitle{Repliement spectral: résumé}
\begin{columns}
\column{60mm}
\begin{center}
\begin{tikzpicture}
...
\end{scope}
	\end{tikzpicture}
\footnotesize{Respect du critère de Shannon}
\end{center}
