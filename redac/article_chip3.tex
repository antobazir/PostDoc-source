...
Modelling of tumor growth  and other cancer tissue properties has been an active research field for the last 50 years various types of model have been used to capture the main feature of various living systems and their interactions with their surroundings. Different types of models are used to describe processes on different spatial and temporal scales. Systems biology, for example, resorted to modelling to describe and understand regulatory network and mean-field models have been used to describe tissue-scale growth and response to environment.\cite{Koch2017}\cite{Watanabe2016}\cite{Jin2021} Use of computational tools to solve the model has progressed along with the available computational power in the last decades, and two types of model have emerged over the years to capture both cell-level response mechanisms and tissue-scale growth and geometry: continuous model and agent-based models.

%Continuous models 
In continuous models most variables are represented as densities or concentrations which are linked by ordinary differential equations and/or partial differential equations. Cell or tissue response to the surrounding environment is then generally described by mathematical functions. Continuous models with few variables do not require the use of computational tools and some of them can even be solved analytically. However, finer modelling and inclusion of coupled can make computational tools necessary to solve the equations. Applications in terms of cancer range from description of bacterial movements to the complex regulatory response of a tissue coupled to growth to spheroid growth or cell migration.\cite{Casciari1992}\cite{Villa2021}\cite{Katsaounis2023}

%Agent-based models
In agent-based models cells (or sometimes groups of cells) are represented as individual agents and algorithmic rules are used instead of mathematical functions to encode cell behavior and interactions with the surrounding environment and one another. In most cancer modelling studies, agent-based models are generally coupled to a set of partial differential equation to describe nutrients of cytokines/chemokines or the evolution of surrounding environment. Agent-based models can be classified into "off-lattice" and "on-lattice" models.\cite{Osborne2017} In "off-lattice" models, the three-dimensional space is represented as triplet of real numbers and cell movement and deformations are calculated by solving Newton's 3rd law. "On-lattice" models represent cells as points on a fixed grid. In general, off-lattice model are favored for phenomenon where stress or any mechanical quantity needs to be known quantitatively. On-lattice models, on the other hand, are simpler and remain useful as long as a coarse discrete description of space does not impact results too negatively. In this study, the characteristics of the system led to the choice of an hybrid on-lattice agent based model which is described herefater.

\subsection{Agent-based modelling for the DMG-on-chip}
%the model
As stated above, the model used in this study is a hybrid on-lattice agent-based model. This model can be broadly understood as a collection of links and interactions between three main grids : The cellular grid, the nutrient grid and the consumption grid.
...
%The nutrient grid
The nutrient grid contains the concentration of nutrient at each vertex. A finite difference algorithm updated the concentration at every timestep. This grid is also checked whenever cell response to the environment is set to occur. In this case, only oxygen dynamics is modelled as no data on glucose was collected. But addition of glucose would be straightforward if needed, as it would operate identically, only with differences in values of concentrations and parameters.

%The consumption grid
This grid is updated regularly to reflect the evolution of cellular nutrient consumptions rates and the impact of their position on the spatial distribution of nutrients. Due to the fact that its evolution needs to be gradual and consistent, the consumption grid is also updated at every step with information from other grid but also the cell table which will be detailed in the next paragraph.

%Other data structures
The aforementioned grids are linked together, but they are also linked to other important data structures in the model such the cell table, which contains the information of all cells in the model sorted by index. This allows to keep track of a cell position and state in straightforward fashion without needing complex scripts to retrieve the needed data in the various grids. It is also the structure that retains the information about dead cells, as it table entry is updated to reflect its state but not deleted. It is also in the cell table that the max consumption rate and its potential evolution are computed before being fed back to the consumption grid. The general data structure of the model is schematically represented in figure \ref{structure_model}

%The evolution 
The model is ran minute-by-minute. Every minute, the model checks whether any modification is necessary on the cellular grid. For example, some cells migrate, divide or commit to death, then the three grids are updated to reflect that change. Once the cellular checks are run, the nutrient grid is updated by running the finite difference algorithm in order to simulate the evolution of the system during a minute.

%Grid structure 
The chosen model structure has the advantage of providing vertex-to-vertex match between all three grids. This avoids any interpolation or calculation of spatial coordinates which reduces computational work compared to on-lattice models and makes the model conducive to parallelization.\\
...
\begin{scope}[scale=1.5]
    ...
    \draw[fill=blue!60!white] (0.4,0.6) grid[step=0.1] (1.1,0.9) rectangle (0.4,0.6);
 	...
 	\draw(0.75,-0.25) node{Cellular grid};
 	...
 	\begin{scope}[yshift=-2cm]
    	...
    	\draw[fill=red!50!white] (0.5,0.5) grid[step=0.1] (1.0,1.0) rectangle (0.5,0.5);
 		...
 		\draw(0.75,-0.25) node{Nutrient grid};
 	\end{scope}
 	...
 	\begin{scope}[yshift=-4cm]
    	...
    	\draw[fill=green!70!black] (0.4,0.6) grid[step=0.1] (1.1,0.9) rectangle (0.4,0.6);
 		...
 		\draw(0.75,-0.25) node{Consumption grid};
 	\end{scope}
 	
 	\draw(3.25,-1.5) circle(0.5) node{Model};

	\draw[->] (1.65,0.65) to (3,-1.0);
	\draw[<-] (1.65,0.25) to (2.8,-1.2);
	
	\draw[->] (1.65,-1.65) to (2.7,-1.65);
	\draw[<-] (1.65,-1.45) to (2.7,-1.45);
	
	\draw[->] (1.65,-3.05) to (2.8,-1.85);
	\draw[<-] (1.65,-3.45) to (3.0,-2.05);
	
	\draw[->] (3.8,-1.65) to (4.8,-1.65);
	\draw[<-] (3.8,-1.45) to (4.8,-1.45);
	%\draw[<-] (-0.25,0.65) to[bend right] (-0.25,0.75-2); 
	%\draw[<-] (-0.25,0.65) to[bend right] (-0.25,0.75-4); 	
 	
 	\begin{scope}
 		\begin{scope}[yshift=-2.2cm,xshift=5cm]
 	 		\draw (0,0) node [above right] {...} (3,0.15) rectangle (0,0) ;
 	 		\draw (0,0.15) node [above right] {...} (3,0.3) rectangle (0,0.15);
 	 		\draw (0,0.3) node [above right] {...} (3,0.45) rectangle (0,0.15);
 	 		\draw (0,0.45) node [above right] {...} (3,0.6) rectangle (0,0.15);
 	 		\draw (0,0.6) node [above right] {...} (3,0.75) rectangle (0,0.15);
 	 		\draw (0,0.75) node [above right] {...} (3,0.9) rectangle (0,0.15);
 	 		\draw (0,0.75) node [above right] {...} (3,0.9) rectangle (0,0.15);
 	 		\draw (0,0.9) node [above right] {...} (3,1.05) rectangle (0,0.15);
 	 		\draw (0,1.05) node [above right] {...} (3,1.20) rectangle (0,0.15);
 	 		\draw (0,1.20) node [above right] {...} (3,1.35) rectangle (0,0.15);
 		\end{scope}
 		\begin{scope}[yshift=-2.2cm,xshift=6.35cm]
 	 		\draw (0,0) node [above right] {...} (3,0.15) rectangle (0,0) ;
 	 		\draw (0,0.15) node [above right] {...} (3,0.3) rectangle (0,0.15);
 	 		\draw (0,0.3) node [above right] {...} (3,0.45) rectangle (0,0.15);
 	 		\draw (0,0.45) node [above right] {...} (3,0.6) rectangle (0,0.15);
...
\subsection{Modelling oxygen dynamics}
...
%[improve with information on PDMS]
...
Before engaging with quantitative aspects, the definition of oxygen consumption rate is briefly reminded. Cell oxygen consumption is a complex process whose characteristics can vary in time as a function of physical and physiological parameters. In computational models, this process is generally divided into two parts. The physical part and the biological part. The physical part correspond to the quotient of concentration in the Hill function and follows a ligand-receptor model. The maximum value, which occurs for "high" concentrations, can change with environmental conditions, cell cycle phase or if a drug such as Oligomycin-A is administered. What is normally referred to as oxygen consumption rate in studies is the maximum value in normal conditions, often referred to as "basal" OCR. 
...
It is however interesting to note that a later study on DIPG cell lines reported an OCR between 200 and 600 pmol/min/cells for SU-DIPG17, which does not seem to match any of the previously reported values.\cite{Shen2024} It seems once again to be a case of poor conversion. It is reported that they used the Seahorse XF protocol with 24-well plates. According to the supplier, cell number in a plate should be approximately 10$^5$ cells. Now if we consider the reported value to be 200-600 pmol/min/1e5 cells instead,  the oxygen consumption rate for SU-DIPG17 celles would correspond to 2000-6000 pmol/min/10$^6$ cells. Comparison data between SU-DIPG17 and HSJD-DIPG-007 showed that HSJD-DIPG-007 consume 2.5 times less oxygen in basal respiration compared to the SU-DIPG17, which would lead to approximately 800-2400 pmol/min/10$^6$ cells for HSJD-DIPG007 and 500-1500 pmol/min/10$^6$ cells for HSJD-DIPG0012. Even though it has been possible to reconcile apparently incompatible values with reasonable assumptions, the variablity of values and their reporting by different authors means that maximum basal OCR, like matrix oxygen diffusion coefficient, has a relatively large range on realistic values. To summarize the various values will be converted into fmol/min/cells and compiled into table \ref{tab:ocr}
...
\hline
 ...
 \hline 
    U87 2D cultures w/ respirometer & 4.8 fmol/min/cell & \cite{Ruas2018}\\
 \hline
...
An important point is that in the model concentrations are given in millimolars. This means concentration cannot be directly injected into the model and must first be converted from fmol/min/cell into mM/min. This conversion requires a volume to convert the moles into a concentration. This volume is taken to be to the lattice site volume which is 8000 µm$^3$. Therefore, a basal oxygen consumption rate of 1 fmol/min/cell corresponds to $k_{max}$ of  0.125 mM/min.
...
The reference configuration is a set of parameters obtained by taking the median value of the different ranges presented previously. All these relevant parameters are presented in table \ref{tab:1}.
...
The experimental data for oxygen concentration has been obtained through the use of foils sensitive to oxygen saturation. The foil technology developed by Presense uses two fluorescent molecules : PtTFPP and N-(5-carboxypentyl)-4-piperidino-1,8-naphthalimide. The fluorescence of PtTFPP is quenched by oxygen which leads to a decrease in fluorescence intensity when the oxygen saturation increases with the second molecule being used as reference for fluorescence intensity. The foil is positioned between the lower coverslip and the medium, PDMS and the cell-containing pellet. The oxygen concentration is measured by acquiring fluorescence images of the sensitive foil overtime and averaging the fluorescence value on the width of the sensitive foil. Therefore, the error bars in the experiment value correponds to the standard deviation of the fluorescence intensity converted into oxygen concentration value. %It is also noted that on available images, the sensitive foil is difficult to properly center relative to the pellet and channel. This is will be accounted for when reproducing the data with simulation.

\begin{figure}[ht!]
\centering
\includegraphics[scale=0.55]{/home/antonybazir/Documents/Post-doc/chip_article_Aless/construction_oxymap.png}\\

\caption{Construction and explanation of the figures for oxygen results \label{oxymap_build}}
\end{figure}

In order to better understand results, the oxygen dynamics results are presented as colormaps. The construction of the colormap is schematically presented in figure \ref{oxymap_build}.  In those maps,
...
\includegraphics[scale=0.33]{/home/antonybazir/Documents/Post-doc/chip_article_Aless/metvar_2D/cons_ko/ref_conf/results/c_OS_hypox_tol/cell_population.png}
\caption{ Map of the oxygen concentration on the center line of the pellet and channel (top) and live cell population (bottom) \label{ref_map}}
\end{figure}

The results on oxygen dynamics live cell population are presented in figure \ref{ref_map}. The first observation is the oscillation of the oxygen concentration in the system with that set of parameters. After a first transient phase of depletion leaving the entire pellet depleted of oxygen after 2 hours, cell response leads to a general decrease in consumption, which results in recovery of oxygen concentration in a couple of hours as well. Cell population sees a corresponding large decrease due to the lack of oxygen in the center region. The oxygen concentration then gradually shifts to an oscillatory pattern with a period of 4 hours over the entire line. After a large decrease in the the first four hours, live cell population decreases slightly again as the system settles into a stationary state. The amplitude of oxygen concentration oscillations is more pronounced in the pellet than in the channel which can be seen in both the color code and the position of the aforementioned limits over time.

The oscillatory concentration occurs due to cells at the rim of the pellet being periodically rescued from starvation. Indeed, with time a large portion of cells in the model are subjected to anoxia ($p_{O_2}>p_{O_2,death}$). As a result, their consumption starts decreasing. After some time, the large number of cells having decreased consumption leads to oxygen concentration in the pellet gradually recovering thanks to the maintained diffusive flux. The oxygen concentration eventually recovers to high enough levels to prevent cell death in a circular patch. As cells are rescued from starvation, consumption recovers, and therefore oxygen levels decrease again until anoxic level is reached, and so on. It is important to keep in mind that cells can be rescued from cell death as long as their consumption does not fall to zero. If cells committed to cell death permanently as soon as they detected a starvation-level oxygen concentrationn the oscillations would not occur and a large proportion of cells would starve in a matter of hours. A direct consequence of the depletion of oxygen in the pellet is that no cell remains able to proliferate due to the oxygen concentration over the entire pellet remaining under the proliferation threshold of  0.1232 mM. This is observable as the live cell population, and therefore cell density are constant after the first 6 hours of simulation. 

The reference configuration is not adapted for reprodution of the experimental dataset but provides a good example of the complex response involving several relaxation times and possible oscillations in terms of oxygen concentration. For instance, it provides a first example of how diffusion and consumption lead to a maximum sustainable population in the pellet and how the cell response time to nutrient availability changes can lead to oscillations in nutrient concentration. These somewhat unrealistic results come from the fact that some parameters were estimated rather than directly measured. In order to understand the complex oxygen dynamics and their link to cell population, model parameters must be varied in order to study the corresponding response in the hopes of being able to reproduce, or at least get closer to experimental data.

\subsection{Impact of cell basal oxygen consumption rate} 
A first parameter studied in order to better understand the role of various parameters in the system is the impact of cell basal oxygen consumption rate on the oxygen dynamics. The basal OCR values tested were 0.2 mM/min and 0.6 mM/min, lower and higher than the reference value of 0.4 mM/min.
...
\begin{subfigure}{0.44\textwidth}
	...
	\includegraphics[scale=0.3]{/home/antonybazir/Documents/Post-doc/chip_article_Aless/metvar_2D/cons_ko/k02/results/c_OS_hypox_tol/cell_population.png}
\end{subfigure}
\begin{subfigure}{0.44\textwidth}
	\centering
	...
	\centering
	\includegraphics[scale=0.3]{/home/antonybazir/Documents/Post-doc/chip_article_Aless/metvar_2D/cons_ko/k06/results/c_OS_hypox_tol/cell_population.png}
\end{subfigure}
\caption{Oxygen concentration on the center line of the pellet (top) and live cell population (bottom) in simulations with $k_{O,max}$ = 0.2 mM/min (left) and $k_{O,max}$ = 0.6 mM/min (right) \label{kcons_map}}
\end{figure}

As can be seen in figure \ref{kcons_map}, the consumption impacts the population and the oxygen dynamics in several ways. As observed in the reference configuration, takes twelve hours for the system to settle into the oscillatory regime and said oscillations still have a 4-hour period. The cell population still follows the same qualitative trend with a short increase, stagnation for a couple of hours as oxygen gets scarce, a firt sharp decrease and then another less pronounced decrease until the stationary level is reached. 

When basal maximum oxygen consumption rate is decreased, the amplitude of the oxygen concentration oscillations decreases in both the pellet and medium as shown in by the color variation is less marked than in the reference configuration. In terms of live cell population, the first decrease is less pronounced than in the reference configuration as more cells survive the first depeletion phase due to the lower overall oxygen consumption. Due to the higher number of cell able to recover to proliferation state, the second decrease is more pronounced than in the reference configuration. After 12 hours the cell population stabilizes to $\approx 6000$ which is higher than the reference configuration ($\approx 5000$). 

Logically, increasing basal oxygen consumption rate reverts all the aforementioned trends compared to the reference configuration. The first decrease is more pronounced, the second decrease is less pronounced and the stable live cell popoulation is lower than in the reference configuration ($\approx 5000$ vs $\approx 4000$).

The delimitation lines between the proliferating, quiescent and starved zones move in a different way. The delimitation line between the proliferating and quiescent zones (white line with triangles in figure \ref{kcons_map} corresponding to $p_{O_2}$= 0.1232mM) is outside the pellet (marked by the dashed red line) and oscillates around a final value of -1.5 mm by  -0.5 mm relative to the delimitation between the pellet and medium. The delimitation line between the quiescent and dead zones (white line with crosses in figure \ref{kcons_map} corresponding to $p_{O_2}$= 0.0704 mM) varies in space with its position oscillating between 0 and 2 mm from the rim of the pellet. The difference of amplitude between the oscillation comes from the difference in consumption rate and and oxygen diffusion coefficient between the two zones. In the channel, diffusion coefficient is high relative to the pellet and consumption is acting indirectly leading to lower amplitudes of the oscillation

In the high consumption case, the oxygen concentration oscillations are more pronounced, as could be expected. The spatial oscillations of the delimiting lines of quiescent and cell death zones are also more pronounced  with this time even less cells remaining in the survival zone when the oxygen concentration is minimal.
...
\begin{subfigure}{0.44\textwidth}
	...
	\includegraphics[scale=0.3]{/home/antonybazir/Documents/Post-doc/chip_article_Aless/metvar_2D/cons_ko/Dmin/results/c_OS_hypox_tol/cell_population.png}
\end{subfigure}
\begin{subfigure}{0.44\textwidth}
	\centering
	...
	\includegraphics[scale=0.3]{/home/antonybazir/Documents/Post-doc/chip_article_Aless/metvar_2D/cons_ko/Dmax/results/c_OS_hypox_tol/cell_population.png}
\end{subfigure}
\caption{Oxygen concentration on the center line of the pellet (top) and live cell population (bottom) in simulations for the $D_{min}$ configuration (left) and the $D_{max}$ configuration (right)  \label{Dminmax_map}}
\end{figure}

The $D_{min}$ configuration corresponds to a lower oxygen mobility than the reference configuration. The amplitude of oscillations of the oxygen concentration is more pronounced than in the reference configuration which is expected. Lowering the diffusive flux while keeping consumption at the same level will result in slower recovery and therefore larger oscillations. The delimitation lines for proliferation and starvation also logically vary more in position than in the reference configuration. In terms of live cell population, the first sharp decrease is more pronounced than in the reference configuration and the second decrease is negligible. This means the consumption never reincreases enough to durably put more cells in the starvation zone. The stable live cell population is slightly below that of the reference configuration ($\approx 4000$ vs $\approx 5000$).

Conversely, if the  overall oxygen mobility is higher than in the reference configuration ($D_{max}$ configuration), the amplitude of the oxygen concentration oscillations is lower, which is visible in the color code in figure \ref{Dminmax_map}. The delimitation line between proliferation and quiescent zones is still outside the pellet but stabilises to a constant position after 12 hours. The behavior of the delimitation line between quiescent and necrotic zone is less intuitive. It can vary in position by almost 4mm in an hour. Due to the higher oxygen mobility concentration gradient decrease more rapidly than in the reference configuration. So when consumption decreases or increases, the tissue-wide response is quicker overall, leading to correspondingly large variations in position of the delimitation lines between quiescent and necrotic zones. In terms of live cell population, the first drop population is less pronounced as the diffusive flux is increased by the larger diffusion coefficient. Similarly to the "low consumption" case, the larger oxygen availabilty after the first drop in population results in more cells being able to resume growth and therefore "maximum" consumption, which in turn results in more cell at the core being starved of oxygen in the second consumption phase compared to the $D_{min}$ and reference configurations.

It is interesting to note that while consumption significantly changed the final stable live cell population level, the difference here is much less noticeable with both configurations having a final live cell population that are both similar and very close to the reference configuration. This underlines that consumption and diffusion do not have exactly the same impact on the response of the system. 

\subsection{Impact of response time}
As a reminder, the cell response time is the time needed for cell to completely transition between two consumption levels ($k_{max}$ in the Hill function), with said max consumption value changing gradually over time. The response time of the reference confgiuration was set to three hours since it was observed that the oxygen concentration was stable over the whole line in the  experiments after 4 hours. A "short response time"-configuration was tested with $t_{reac}$ set at 1 hour. For the "long response time"- configuration, $t_{reac}$ was increased to 8 hours.
...
\begin{subfigure}{0.44\textwidth}
	...
	\includegraphics[scale=0.3]{/home/antonybazir/Documents/Post-doc/chip_article_Aless/metvar_2D/cons_ko/reac1/results/c_OS_hypox_tol/cell_population.png}
\end{subfigure}
\begin{subfigure}{0.44\textwidth}
	\centering
	...
	\includegraphics[scale=0.3]{/home/antonybazir/Documents/Post-doc/chip_article_Aless/metvar_2D/cons_ko/reac8/results/c_OS_hypox_tol/cell_population.png}
\end{subfigure}
\caption{Oxygen concentration on the center line of the pellet (top) and live cell population (bottom) in simulations for the a "short cell response time" configuration (left) and "long cell response time" configuration (right)  \label{reac_map}}
\end{figure}

In the short response time configuration, cells deplete the system of oxygen as rapidly as before. The main difference with the reference configuration being that the consumption level falls to zero in an hour. It should be reminded here that he model is coded so that when a cell detects starvation levels of oxygen, it starst decreasing its consumption rate $k_{max}$ gradually (in  $t_{reac}$ minutes), and dies if that level reaches zero before the oxygen concentration reincreases above the starvation level, the cell dies. This means that if $t_{reac}$ is 60 minutes, cells can only survive for 60 minutes. This is what happens in the "short response time"-configuration and why a majority of cells end up dying compared to the reference configuration. In the reference configuration, the recovery of oxygen concentration is sufficient in the 4 hours-window to prevent cell death in 4000 cells. Here, almost all cells die with only less than 1500 cells remaining. Due to the low remaining live cell population, the overall oxygen concentration reincreases. Even though the population seems constant it is actually increasing slightly between 24 and 48 hours. This is due to the a small portion of the pellet having enough oxyen to proliferate between 10 and 30 hours.
%hwoever cellular oxygen consumption decreases to zero in an hour, leading to a vast majority of cells dying as diffusion flux alone is not high enough to restore oxyen levels in the matrix before they commit to cell death.

In the 'long response time"-configuration, it is interesting to note that while the first response indeed clearly takes 8 hours, the other feature in the oxygen dynamics are not on the same time scale. In fact, the oscillations visible in the concentration after 24 hours have a period that is closer to 3 hours at the center and in the channel, suggesting that as for other parameters response time has a complex impact on oxygen dynamics that depends a lot on other parameters. In terms of live cell population, the first large popultion decreases corresponds to the consequence of the oxygen concentration being below the starvation on the entire and pellet. As before the subsequent decrease in consumption results in recovery. This happens a second time at a lower scale until the concentration reaches an oscillatory stable regime where no cell proliferate but an area where survival is possible is formed between 0 and 0.5 mm from the rim. This leads to a stable population of $\approx 4000$ cells in the pellet at 48 hours.

\subsection{Proliferation and cell death threshold}
So far, no analysis was done on proliferation data has been done due to the $p_{O_2,prol}$ and $p_{O_2,death}$ threshold values being too high for any proliferation to take place. Lowering those thresholds in order to restore proliferation seems straightforward. However, lowering both threshold means that consumption will then be maintained potentially not yielding the expected effect of restoring proliferation in the pellet.

\begin{figure}[ht!]
\begin{subfigure}{0.44\textwidth}
	\centering
	\includegraphics[scale=0.33]{/home/antonybazir/Documents/Post-doc/chip_article_Aless/metvar_2D/cons_ko/plim5_panox2/results/c_OS_hypox_tol/lines_O_2.png}
	\includegraphics[scale=0.3]{/home/antonybazir/Documents/Post-doc/chip_article_Aless/metvar_2D/cons_ko/plim5_panox2/results/c_OS_hypox_tol/cell_population.png}
\end{subfigure}
\begin{subfigure}{0.44\textwidth}
	\centering
	\includegraphics[scale=0.33]{/home/antonybazir/Documents/Post-doc/chip_article_Aless/metvar_2D/cons_ko/ref_conf/results/c_OS_hypox_tol/lines_O.png}
	\includegraphics[scale=0.3]{/home/antonybazir/Documents/Post-doc/chip_article_Aless/metvar_2D/cons_ko/ref_conf/results/c_OS_hypox_tol/cell_population.png}
\end{subfigure}
\caption{Oxygen concentration on the center line of the pellet (top) and live cell population (bottom) in simulations for the a "short cell response time" configuration (left) and "long cell response time" configuration (right)  \label{thres_map}}
\end{figure}

In figure \ref{thres_map} the results for the lower threshold configuration are compared to the reference configuration. The Lowering $p_{O_2,prol}$ and $p_{O_2,death}$ does not result in the proliferation threshold moving to end up in the matrix so proliferation is still very low.
...
Now that agreement with experimental data has been shown, the oxygen dynamics and population data can be analyzed in more details in order to understand how the different parameters play a role in the results. It can be seen that the population grows overall but cells at the core are dying as the rim population increases. The increasing cell density at the rim leads to a shrinkage of the proliferation zone. The oxygen concentration at the core undergoes oscillations over time due which are most visible between 0 and 10 hours.
...
\includegraphics[scale=0.33]{/home/antonybazir/Documents/Post-doc/chip_article_Aless/metvar_2D/cons_ko/best/results/c_OS_hypox_tol/cell_population.png}
\caption{ Map of the oxygen concentration on the center line of the pellet (top) and live cell population (bottom) and channel for the optimal set of parameters \label{best_map}}
\end{figure}
...
%The difficulties in reproducing the agreement obtained at 10000 cells/µL at A lower cell density are also interesting. The discrepancies that appear at lower cell densities suggest that the model in its current state cannot capture the complexity of cellular responses to the environment. It is possible that the oxygen consumption rate and general cellular behavior has a complexe relation to cell density or even matrix density. Such relation would need to be further studied in different experiments. 
