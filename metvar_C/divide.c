/*this divide is coded specially for the travelling wave code*/
#include <stdio.h>
#include <errno.h>
#include <stdlib.h>
#include "structdef.h"
...
int divide(int cell_idx) 
{
	...
	row = Mod.M_Cell.state[cell_idx-1][7];
	col = Mod.M_Cell.state[cell_idx-1][8];
	...
	if(n_free==0)
	{
		...
		while(interm_row!=row||interm_col!=col) 
		{
			...
			Mod.M_Cell.state_mat[interm_row+row_shift[chosen_site]][interm_col+col_shift[chosen_site]] = Mod.M_Cell.state_mat[interm_row][interm_col];
		...
		/*state vector update*/
		Mod.M_Cell.state[Mod.M_Cell.N_Cell][0] = 1.0;
		Mod.M_Cell.state[cell_idx-1][0] = 1.0; /*divison timer*/
		...
		Mod.M_Cell.state[Mod.M_Cell.N_Cell][10] = Mod.M_Cell.state[cell_idx-1][10];
		...
		Mod.M_Cell.state_mat[interm_row+row_shift[chosen_site]][interm_col+col_shift[chosen_site]] = Mod.M_Cell.state_mat[row][col];
	...
	{	
		...
		Mod.M_Cell.state[Mod.M_Cell.N_Cell][0] = 1.0; /*divison timer*/
		Mod.M_Cell.state[cell_idx-1][0] = 1.0; 
		...
		Mod.M_Cell.state[Mod.M_Cell.N_Cell][5] = Mod.M_Cell.state[cell_idx-1][5];/*oxygen consumption*/
		Mod.M_Cell.state[Mod.M_Cell.N_Cell][6] = Mod.M_Cell.state[cell_idx-1][6];/*oxygen consumption evolution*/
		Mod.M_Cell.state[Mod.M_Cell.N_Cell][7] = (float)(nf_row[0]);/*row*/
		Mod.M_Cell.state[Mod.M_Cell.N_Cell][8] = (float)(nf_col[0]);/*column*/
		Mod.M_Cell.state[Mod.M_Cell.N_Cell][9] = (float)(cell_idx);/*parent*/
		Mod.M_Cell.state[Mod.M_Cell.N_Cell][10] = Mod.M_Cell.state[cell_idx-1][10];
...
/*division routine with random clock*/
int divide_s(int cell_idx) 
{
	...
	if(n_free==0)
	{
		...
		while(Mod.M_Cell.Grid[interm_row+row_shift[chosen_site]][interm_col+col_shift[chosen_site]]!=0)
		{
			interm_row = interm_row+row_shift[chosen_site];
			interm_col = interm_col+col_shift[chosen_site];
			if(Mod.M_Cell.Grid[interm_row+row_shift[chosen_site]][interm_col+col_shift[chosen_site]]==-1)
			{
				/*si on tombe hors de la matrice on reprend dans une autre direction*/
				chosen_site = (rand() %	(7 - 0 + 1)) + 0;
				interm_row = row ;  interm_col = col;
		...
		/*state vector update*/
		Mod.M_Cell.state[Mod.M_Cell.N_Cell][0] = 1.0+(float)((rand() % (60 - 0 + 1)) + 0);
		Mod.M_Cell.state[cell_idx-1][0] = 1.0; /*divison timer*/
	...
	{	
		...
		Mod.M_Cell.state[Mod.M_Cell.N_Cell][0] = 1.0+(float)((rand() % (60 - 0 + 1)) + 0); /*divison timer*/
		Mod.M_Cell.state[cell_idx-1][0] = 1.0; 
