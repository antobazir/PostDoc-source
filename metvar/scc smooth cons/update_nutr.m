...
function [M1,kM1,DM1m,M2,kM2,DM2m,M1t,M2t] = update_nutr(Grid,LD,M1,kM1,DM1m,k1_tss,M2,kM2,DM2m,k2_tss,bhv,d0,ntime,tau,dx,dt)
sz  = size(M1,1);
%disp('cons_started')
...
B = bhv*ones(sz,sz);
[kM1,kM2] = arrayfun(@cons_map,M1,M2,B);
...
##  M(:,sz) = M(:,sz-1);
    ...
    %x-step
    M1(2:sz-1,:) =  M1(2:sz-1,:) + d0^2*dt/tau*(DM1m(3:sz,:)-DM1m(1:sz-2,:))/(4*dx^2).*(M1(3:sz,:)-M1(1:sz-2,:)) + DM1m(2:sz-1,:)*d0^2*dt/tau/dx^2.*(M1(3:sz,:) -2*(M1(2:sz-1,:)) + M1(1:sz-2,:)) -k1_tss*LD(2:sz-1,:).*kM1(2:sz-1,:).*dt;
  %y-step
    M1(:,2:sz-1) =  M1(:,2:sz-1) +  d0^2*dt/tau*(DM1m(:,3:sz)-DM1m(:,1:sz-2))/(4*dx^2).*(M1(:,3:sz)-M1(:,1:sz-2)) + DM1m(:,2:sz-1)*d0^2*dt/tau/dx^2.*(M1(:,3:sz) -2*(M1(:,2:sz-1)) + M1(:,1:sz-2));
        ...
        %x-step
    M2(2:sz-1,:) =  M2(2:sz-1,:) + d0^2*dt/tau*(DM2m(3:sz,:)-DM2m(1:sz-2,:))/(4*dx^2).*(M2(3:sz,:)-M2(1:sz-2,:)) + DM2m(2:sz-1,:)*d0^2*dt/tau/dx^2.*(M2(3:sz,:) -2*(M2(2:sz-1,:)) + M2(1:sz-2,:)) -k2_tss*LD(2:sz-1,:).*kM2(2:sz-1,:).*dt;
  %y-step
    M2(:,2:sz-1) =  M2(:,2:sz-1) +  d0^2*dt/tau*(DM2m(:,3:sz)-DM2m(:,1:sz-2))/(4*dx^2).*(M2(:,3:sz)-M2(:,1:sz-2)) + DM2m(:,2:sz-1)*d0^2*dt/tau/dx^2.*(M2(:,3:sz) -2*(M2(:,2:sz-1)) + M2(:,1:sz-2));
 ...
 %delta = max(max(abs(M-prev_M)));
	...
	M2t(:,i) = M2(1:round(sz/2),round(sz/2));
