...
function[Grid,LD,kO,kS,kP,DSm,DOm,DPm,DKm,state] = divide2D_metvar(Grid,LD,pos,c_idx,sz,S,P,O,K,state,kO,kS,kP,kO_tissue,kS_tissue,kP_tissue,DOm,DSm,DPm,DKm,DOx_tissue,DS_tissue,DP_tissue,DK_tissue)%on autorise la diffusion sur les 8 voisins et plus seulement sur 4
%plr = randi(1:4);
pos;
    ...
    % Si la case d'à côté est vide on définit une position intermédiaire
      ...
      %on retourne le vecteur
			...
			for i=1:size(shift_vec,1)
				...
				LD(shift_vec(i,1)-1,shift_vec(i,2)-1) = LD(shift_vec(i,1),shift_vec(i,2));
    ...
    case 2
		if(Grid(pos(1)-1,pos(2))!=0)
			...
			for i=1:size(shift_vec,1)
				...
				LD(shift_vec(i,1)-1,shift_vec(i,2)) = LD(shift_vec(i,1),shift_vec(i,2));
	...
	case 4
		...
		if(Grid(pos(1),pos(2)-1)!=0)
			...
			for i=1:size(shift_vec,1)
				...
				LD(shift_vec(i,1),shift_vec(i,2)-1) = LD(shift_vec(i,1),shift_vec(i,2));
