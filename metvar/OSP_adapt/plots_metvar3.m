...
figure
imagesc(dx*(-round(sz/2)+1:round(sz/2))*1e-3,dx*(-round(sz/2)+1:round(sz/2))*1e-3,Grid_r{1669})
ylabel('position (mm)')
xlabel('position (mm)')
set(gca,'outerposition',[0 0.05 0.9 0.9])
...
for i=1:size(S_r,2)
  S_ctr(i) = S_r{i}(15,15);
  DSm_ctr(i) = DSm_r{i}(15,15);
  kS_ctr(i) = kS_r{i}(15,15);
endfor
plot((1:size(S_r,2)),S_ctr)
...
for i=1:size(O_r,2)
  O_ctr(i) = O_r{i}(15,15);
  %DOm_ctr(i) = DOm_r{i}(15,15);
  kO_ctr(i) = kO_r{i}(15,15);
endfor
plot((1:size(O_r,2)),O_ctr)
%plot((1:size(DOm_r,2)),DOm_ctr)
plot((1:size(kO_r,2)),kO_ctr)
...
for i=1:size(P_r,2)
  P_ctr(i) = P_r{i}(15,15);
  %DOm_ctr(i) = DOm_r{i}(15,15);
  kP_ctr(i) = k_r{i}(15,15);
endfor
plot((1:size(P_r,2)),P_ctr)
%plot((1:size(DOm_r,2)),DOm_ctr)
plot((1:size(kP_r,2)),kP_ctr)
...
figure
hold on
plot(dx*(-round(sz/2)+1:round(sz/2))*1e-3,S_r{145}(round(sz/2),:))
plot(dx*(-round(sz/2)+1:round(sz/2))*1e-3,O_r{145}(round(sz/2),:))
ylabel('Concentration')
xlabel('position (mm)')
set(gca,'outerposition',[0 0.05 0.9 0.9])
