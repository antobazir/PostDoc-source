...
figure
%P_r = reshape(P_r, [size(P_r,1) size(P_r,1) size(P_r,2)/size(P_r,1)]);
for i=1:size(P_r,2)
  i
  imagesc(P_r{i});colorbar
  caxis([0 1])
  kbhit
endfor
...
figure;
plot(linspace(0,n_pt,length(Ot_r(25,:))),Ot_r(25,:))
