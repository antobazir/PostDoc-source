...
kS_arg(2) =  kS_; kO_arg(2) = kO_/10; filename(2,:) = [folder_name '/Ox'];
kS_arg(3) =  kS_/10; kO_arg(3) = kO_; filename(3,:) = [folder_name '/Gl'];
...
aF = axes(F);
hold on;
plot(pos,Pmap(round(sz(:,:,:,3)/2),:,size(Grids,3)-4),'color','blue')
plot(pos,Pmap(round(sz(:,:,:,3)/2),:,size(Grids,3)-2),'color','red')
plot(pos,Pmap(round(sz(:,:,:,3)/2),:,size(Grids,3)),'color','green')
line([0 0],[0 1.2],'color','black')
xlabel(aF,'position (mm)','fontsize',20)
ylabel(aF,'Prod. concentration','fontsize',20)
ylim([0 1.2])
legend(aF,['d' num2str(size(Grids,3)-4)],['d' num2str(size(Grids,3)-2)],['d' num2str(size(Grids,3))],'location','northwest');
text(round(sz(:,:,:,3)/3).*dx(:,:,:,3)*1e-3,0.5,'high subs','horizontalalignment','center')
text(-round(sz(:,:,:,3)/3).*dx(:,:,:,3)*1e-3,0.5,'low subs','horizontalalignment','center')
set(aF,'outerposition',[0.05 0.05 0.95 0.95])
print(F,["/home/antony/Documents/Post-doc/Redac/metvar/" folder_name "_Prod.pdf"], "-dpdflatex","-S240,180","-FCalibri:24");

F = figure('visible','off');
aF = axes(F);
hold on;
plot(pos,Kmap(round(sz(:,:,:,3)/2),:,size(Grids,3)-4),'color','blue')
plot(pos,Kmap(round(sz(:,:,:,3)/2),:,size(Grids,3)-2),'color','red')
plot(pos,Kmap(round(sz(:,:,:,3)/2),:,size(Grids,3)),'color','green')
line([0 0],[0 1.2],'color','black')
xlabel(aF,'position (mm)','fontsize',20)
ylabel(aF,'Kin. concentration','fontsize',20)
ylim([0 1.2])
legend(aF,['d' num2str(size(Grids,3)-4)],['d' num2str(size(Grids,3)-2)],['d' num2str(size(Grids,3))],'location','northwest');
text(round(sz(:,:,:,3)/3).*dx(:,:,:,3)*1e-3,0.5,'high subs','horizontalalignment','center')
text(-round(sz(:,:,:,3)/3).*dx(:,:,:,3)*1e-3,0.5,'low subs','horizontalalignment','center')
set(aF,'outerposition',[0.05 0.05 0.95 0.95])
print(F,["/home/antony/Documents/Post-doc/Redac/metvar/" folder_name "_Kin.pdf"], "-dpdflatex","-S240,180","-FCalibri:24");

F = figure('visible','off');
aF = axes(F);
hold on;
plot(1:size(St,2),St(round(sz/2),:,size(St,3)));
plot(1:size(Kt,2),Kt(round(sz/2),:,size(Kt,3)));
xlabel(aF,'time (min)','fontsize',20)
ylabel(aF,'Concentration','fontsize',20)
set(aF,'outerposition',[0.05 0.05 0.95 0.95])
print(F,["/home/antony/Documents/Post-doc/Redac/metvar/" folder_name "_relax.pdf"], "-dpdflatex","-S240,180","-FCalibri:24");
