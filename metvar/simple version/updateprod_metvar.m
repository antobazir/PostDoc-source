...
function [M,kM,DMm,Mt,delta] = updateprod_metvar(Grid,M_s,M,kM,cM,DMm,d0,ntime,tau,dx,dt)
sz  = size(M,1);
...
##    M(:,2:sz-1) =  M(:,2:sz-1) + DMm(:,2:sz-1)*dt/dx^2.*(M(:,3:sz) -2*(M(:,2:sz-1)) + M(:,1:sz-2));
  ...
  prev_M = M;%
    ...
    %x-step
    M(2:sz-1,:) =  M(2:sz-1,:) + d0^2*dt/tau*(DMm(3:sz,:)-DMm(1:sz-2,:))/(4*dx^2).*(M(3:sz,:)-M(1:sz-2,:)) + DMm(2:sz-1,:)*d0^2*dt/tau/dx^2.*(M(3:sz,:) -2*(M(2:sz-1,:)) + M(1:sz-2,:)) -V_norm*kM(2:sz-1,:).*dt.*M_s(2:sz-1,:)./(cM + M_s(2:sz-1,:));
  %y-step
