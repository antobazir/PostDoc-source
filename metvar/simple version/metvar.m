...
n_min = 30; %  15 conf(j+1);j++; % simulated time in minutes
dx = 20; % 20  conf(j+1);j++;
dt =  15e-4;%conf(j+1);j++;
...
%consumption
kO_tissue = 0.7%conf(j+1);j++; %maxi value of consumption term for oxygen
...
while(n_cy<25) % loop to stop at a given size
	n_cy++;
	div =0;
	...
	if(n_cy>n_init)
		...
		%[P,kP,DPm,Pt] = update_metvar(Grid,P,kP,cP,DPm,d0,tau,ntime,dx,dt);
		toc
		...
		state(Grid(find(and(S<S_maint,Grid!=0))))=-1;
	...
	%imagesc(Grid!=0)
		...
		figure 1
		contour(((1:sz)-sz/2)*dx,((1:sz)-sz/2)*dx,S)
		caxis([0 1])
		colorbar
		figure 2
		contour(((1:sz)-sz/2)*dx,((1:sz)-sz/2)*dx,O)
		caxis([0 1])
		colorbar
		figure 3
		imagesc(((1:sz)-sz/2)*dx,((1:sz)-sz/2)*dx,Grid!=0)
		figure 4
		imagesc(((1:sz)-sz/2)*dx,((1:sz)-sz/2)*dx,kS)
	  colorbar
	  caxis([0 kS_comp])
		figure 5
		imagesc(((1:sz)-sz/2)*dx,((1:sz)-sz/2)*dx,kO)
	  colorbar
	  caxis([0 kO_tissue])
		figure 6; hold on;
		plot(S(:,round(sz/2)),'-x')
