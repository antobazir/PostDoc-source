    ...
    if(Grid(ng(plr,1),ng(plr,2))!=0)
     ...
     % disp('while')
      ...
      for i=1:size(shift_vec,1)
        ...
        d = sqrt((ng(plr,1)-round(sz/2)).^2+(ng(plr,2)-round(sz/2)).^2);
        cycle_duration = 1; % round(interp1([0.150 0.0375 0.015],[2.8000-0.6 6.6000-0.6 400.0000-0.6]*24*60,O(find(Grid==c_idx)))+864);
        state(size(state,1)+1,:) = [0 state(c_idx,2) state(c_idx,3) state(c_idx,4) state(c_idx,5) state(c_idx,6) state(c_idx,7), d];        % A priori faudrait faire baisser et remonter le conso lors des divisions mais non.
...
##        endif
        ...
        cycle_duration = 1; % round(interp1([0.150 0.0375 0.015],[2.8000-0.6 6.6000-0.6 400.0000-0.6]*24*60,O(find(Grid==c_idx)))+864);
        d = sqrt((ng(plr,1)-round(sz/2)).^2+(ng(plr,2)-round(sz/2)).^2);
        state(size(state,1)+1,:) = [0 state(c_idx,2) state(c_idx,3) state(c_idx,4) state(c_idx,5) state(c_idx,6) state(c_idx,7), d];
