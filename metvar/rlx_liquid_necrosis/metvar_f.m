...
state = [0 1 0 kS_tissue 0 kO_tissue 0 0];
l=1;
...
while(Radius_agg<1000&&n_pt/n_min<10) % loop to stop at a given size here it is  1000 µm
  %disp('******************')
	n_pt++;
...
for k = 1:size(state,1)
  ...
  state(k,8) = sqrt((pos(1)-round(sz/2)).^2 + (pos(2)-round(sz/2)).^2);
